...
- **Blocking watch commands (`gh run watch`, `gh pr checks --watch`) are denied** at the proxy choke point — they wait on an external event and cannot finish within the proxy's single-shot execution budget (`timeouts.cli`, default 60s), so they could only ever die by timeout, which looks exactly like the watched run failing. Poll instead: `gh run view <run-id> --json status,conclusion` in a sleep loop until status is `completed`. Any other command that outlives the budget is killed with **exit code 124** and a stderr note attributing the kill to the proxy (the command itself did not fail).
